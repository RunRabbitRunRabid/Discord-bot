...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o
      .setName('subject1')
      .setDescription('First subject your character excels at')
      .setRequired(true)
      .addChoices(...CLASS_CHOICES)
    )
    .addStringOption(o => o
      .setName('subject2')
      .setDescription('Second subject your character excels at')
      .setRequired(true)
      .addChoices(...CLASS_CHOICES)
    )
    .addStringOption(o => o
  ...
  async execute(interaction) {
    ...
    const prof1  = interaction.options.getString('subject1');
    const prof2  = interaction.options.getString('subject2');
    const act    = interaction.options.getString('afterschool');
    const money  = interaction.options.getNumber('startingmoney');

    if (prof1 === prof2) {
      return interaction.reply({
        embeds: [errorEmbed(
          'Duplicate Subjects',
          `You picked **${prof1}** for both slots. Each character needs two *different* subjects.\n\n` +
          `**Try:** Select a different class for Subject 2.`
        )],
        ephemeral: true,
      });
    }
    ...
    if (allChars.length >= 10) {
      return interaction.reply({
        embeds: [errorEmbed(
          ...
          `**Try:** Use \`/delete\` to remove a character you no longer need, then try again.`
        )],
        ephemeral: true,
      });
    }
    ...
    try {
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: 'Subject 1',      value: prof1,                                      inline: true  },
              { name: 'Subject 2',      value: prof2,                                      inline: true  },
              { name: '\u200b',         value: '\u200b',                                   inline: true  },
              { name: 'After-school',   value: act === 'work' ? 'Part-time Work' : 'Club', inline: true  },
              { name: 'Starting Funds', value: formatMoney(money),                         inline: true  },
              { name: 'Starting XP',   value: '0 XP',                                     inline: true  },
            )
            .setFooter({ text: `Created by ${interaction.user.tag} · Hero Academy` })
            .setTimestamp(),
        ],
      });
    } catch (err) {
      ...
      await interaction.reply({
        embeds: [errorEmbed(
          ...
          `**Try:** Run \`/create\` again in a moment.`
        )],
        ephemeral: true,
      });
    }
